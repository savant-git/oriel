...
def ask_path():
    console.print("📄 Enter path to conversations.json:")
    p = input("→ ").strip()
    return Path(p)

def choose_scope():
    console.print("\nChoose extraction scope:")
    console.print("1. Current chat only\n2. Project chats\n3. All chats")
...
def parse_json(path,scope):
    ...
    rule_status(f"✅ Saved formatted chat → {out_fp}", "ok")
...
def make_export(chat_fp):
    ...
    console.print(f"📦 Creating export → {zip_fp}")
    with zipfile.ZipFile(zip_fp,"w",compression=zipfile.ZIP_DEFLATED) as z:
        z.write(chat_fp,chat_fp.relative_to(BASE))
    rule_status("✅ Export complete and chat embedded.", "ok")
    return zip_fp

def main():
    path=ask_path()
    if not path.exists():
        rule_status("❌ File not found.", "error")
        return
    scope=choose_scope()
