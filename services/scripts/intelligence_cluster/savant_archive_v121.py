...
"""
⬢ Savant Archive v121
Robust chat extraction + export + upload.
Adds full null-safe parsing, path correction, metadata logging.
"""
    ...
    for chat in chats:
        ...
        for node in mapping.values():
            ...
            if text.strip():
                out.append(f"\n[{role}]\n{text.strip()}")
    if not out:
        log("⚠️  No readable content found.")
        return None
...
# --- 5. Main --------------------------------------------------------------
def main():
    json_path = input("📄 Enter path to conversations.json: ").strip() or "/storage/emulated/0/Download/conversations.json"
    jp = Path(json_path)
    if not jp.exists():
        log("❌ File not found.")
        return
